...
#include "dcmtk/dcmjp2k/dj2kcodecd.h"
...
#include "dcmtk/dcmjp2k/dj2kparam.h" /* for class DJ2KCodecParameter */
    ...
    Uint16 pixelRepresentation)
{
  ...
  DCMJP2K_DEBUG("decodeJPEG2000: j2kSize=" << j2kSize << ", bufSize=" << bufSize 
  ...
  if (j2kSize >= 4)
  {
    DCMJP2K_DEBUG("First 4 bytes of J2K data: 0x" 
  ...
  if (j2kSize >= 12 && memcmp(j2kData, JP2_SIGNATURE, 12) == 0)
  {
    ...
    DCMJP2K_DEBUG("Detected JP2 file format");
  }
  else if (j2kSize >= 2 && memcmp(j2kData, J2K_SOC_MARKER, 2) == 0)
  {
    codecFormat = OPJ_CODEC_J2K;
    DCMJP2K_DEBUG("Detected J2K codestream format");
  }
  else
  {
    DCMJP2K_ERROR("Unable to determine JPEG 2000 codec format - first bytes don't match known signatures");
  ...
  if (!codec)
  {
    DCMJP2K_ERROR("Failed to create OpenJPEG decompressor");
    return EC_J2KInvalidCompressedData;
  }
  ...
  {
    DCMJP2K_ERROR("Failed to setup OpenJPEG decoder");
  ...
  if (!stream)
  {
    DCMJP2K_ERROR("Failed to create OpenJPEG stream");
    opj_destroy_codec(codec);
    return EC_J2KInvalidCompressedData;
  }
  ...
  {
    DCMJP2K_ERROR("Failed to read JPEG 2000 header");
  ...
  DCMJP2K_DEBUG("JPEG 2000 image: " << width << "x" << height << ", " << image->numcomps << " components");
  for (OPJ_UINT32 c = 0; c < image->numcomps; c++)
  {
    DCMJP2K_DEBUG("  Component " << c << ": " << image->comps[c].w << "x" << image->comps[c].h
  ...
  {
    DCMJP2K_ERROR("JPEG 2000 image dimensions mismatch: expected "
  ...
  {
    DCMJP2K_ERROR("Failed to decode JPEG 2000 image");
    opj_image_destroy(image);
    opj_stream_destroy(stream);
    opj_destroy_codec(codec);
    return EC_J2KInvalidCompressedData;
  ...
  {
    DCMJP2K_WARN("opj_end_decompress failed, but image may still be valid");
  }
  ...
  DCMJP2K_DEBUG("Component dimensions: " << compWidth << "x" << compHeight << ", pixelCount=" << pixelCount);

  // Check if component data is valid
  if (image->comps[0].data == NULL)
  {
    DCMJP2K_ERROR("Component data is NULL after decoding");
    opj_image_destroy(image);
    opj_stream_destroy(stream);
    opj_destroy_codec(codec);
    return EC_J2KInvalidCompressedData;
  ...
  if (pixelCount > 0)
  {
    ...
    DCMJP2K_DEBUG("Component 0 value range (sampled): min=" << minVal << ", max=" << maxVal);
  }
  ...
  {
    DCMJP2K_ERROR("Output buffer too small: need " << requiredSize << " bytes, have " << bufSize);
    opj_image_destroy(image);
    opj_stream_destroy(stream);
    opj_destroy_codec(codec);
  ...
  DCMJP2K_DEBUG("Successfully decoded " << pixelCount << " pixels to buffer");
    ...
    OFBool& removeOldRep) const
{
  ...
  {
    DCMJP2K_WARN("cannot decompress image because uncompressed representation would exceed maximum possible size of PixelData attribute");
  ...
  while (result.good() && !done)
  {
      ...
      if ((result == EC_J2KInvalidCompressedData) && forceSingleFragmentPerFrame)
      {
        ...
        DCMJP2K_WARN("JPEG 2000 bitstream invalid or incomplete, ignoring (but image is likely to be incomplete)");
        result = EC_Normal;
      }
    ...
    Uint16 bytesPerSample)
{
  ...
  if (imageSamplesPerPixel > 1)
  {
    ...
    if (dataset->findAndGetUint16(DCM_PlanarConfiguration, imagePlanarConfiguration).good() && (imagePlanarConfiguration != 0))
      DCMJP2K_WARN("invalid value for PlanarConfiguration " << DCM_PlanarConfiguration << ", should be \"0\"");
  ...
  if (result.good())
  {
    ...
    if (result.good())
    {
      ...
      if (imageSamplesPerPixel == 3 && imagePlanarConfiguration == 1)
      {
        ...
        DCMJP2K_DEBUG("converting to planar configuration 1");
        if (bytesPerSample == 1)
    ...
    OFString & decompressedColorModel) const
{
  ...
  if (dataset != NULL)
  {
    ...
    {
        DCMJP2K_WARN("mandatory element PhotometricInterpretation " << DCM_PhotometricInterpretation << " is missing");
        result = EC_MissingAttribute;
    }
    else if (result.bad())
    {
        DCMJP2K_WARN("cannot retrieve value of element PhotometricInterpretation " << DCM_PhotometricInterpretation << ": " << result.text());
    }
    else if (decompressedColorModel.empty())
    {
        DCMJP2K_WARN("no value for mandatory element PhotometricInterpretation " << DCM_PhotometricInterpretation);
