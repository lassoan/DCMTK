...
#ifndef DCMJP2K_DJ2KCODECD_H
#define DCMJP2K_DJ2KCODECD_H
 ...
 */
class DCMTK_DCMJP2K_EXPORT DJ2KDecoderBase: public DcmCodec
{
public:
...
private:
  ...
  /** returns true if the transfer syntax supported by this codec
   *  is lossless.
   ...
   *  frame in the compressed pixel sequence.
  ...
  /** check whether the given buffer contains a JPEG 2000 start-of-codestream marker
   *  @param fragmentData pointer to 4 or more bytes of JPEG 2000 data
   *  @param fragmentLength length of the fragment
   *  @returns true if the fragment starts with a JPEG 2000 SOC marker
   */
  static OFBool isJPEG2000StartOfCodestream(Uint8 *fragmentData, Uint32 fragmentLength);

  /** converts an RGB or YBR frame with 8 bits/sample from
   *  color-by-pixel to color-by-plane planar configuration.
   *  @param imageFrame pointer to image frame
 ...
 */
class DCMTK_DCMJP2K_EXPORT DJ2KLosslessDecoder : public DJ2KDecoderBase
{
  /** returns the transfer syntax that this particular codec
   *  is able to decode
   *  @return supported transfer syntax
...
/** codec class for JPEG 2000 (lossy or lossless) TS decoding
 */
class DCMTK_DCMJP2K_EXPORT DJ2KDecoder : public DJ2KDecoderBase
{
  /** returns the transfer syntax that this particular codec
   *  is able to decode
   *  @return supported transfer syntax
